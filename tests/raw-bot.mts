import { Public, Bot } from "../src/raw-clients.mts";
import { ApplicationCommandType, AutomodActionType, AutomodEventType, AutomodTriggerType, ChannelTypes, GuildExplicitContentFilterTypes, GuildFeatures, GuildOnboardingMode, GuildScheduledEventEntityTypes, GuildScheduledEventPrivacyLevels, MetadataItemTypes, RecurrenceRuleFrequencies, StageInstancesPrivacyLevels, VerificationLevels, type SnowflakeType } from "../src/raw-types.mts";
...
export async function testGuildEvents(guild_id: SnowflakeType) {
    ...
    const event = await bot.createGuildScheduledEvent(guild_id, {
        entity_type: GuildScheduledEventEntityTypes.EXTERNAL,
        name: "test",
        scheduled_start_time: new Date(start).toISOString(),
        privacy_level: GuildScheduledEventPrivacyLevels.GUILD_ONLY,
        entity_metadata: {
            location: "test"
        },
